...
Using YOLO26s + OpenCV + Streamlit
"""
...
def load_yolo_model():
    ...
    try:
        ...
        model_path = Path("models/yolo26s_traffic.pt")
        if model_path.exists():
            model = YOLO(str(model_path))
        else:
            # 事前学習済みyolo26s をベースに使用
            model = YOLO("yolo26s.pt")
            st.warning("⚠️ カスタムモデルが見つかりません。デモ用にyolo26s.ptを使用しています。")
        return model
    except ImportError:
...
<div class="hero">
  <h1>🚦 信号機判別アプリ</h1>
  <p>YOLO26s + OpenCV で信号機を検出・色判別し、音声でお知らせします</p>
</div>
""", unsafe_allow_html=True)
...
# タブ構成
tab1, tab2, tab3 = st.tabs(["📹 カメラ撮影", "📷 画像アップロード", "ℹ️ 使い方"])

# ── タブ1: カメラ撮影（最初に開く） ───────────────────
with tab1:
    # 背面カメラ指定 + カメラ表示を大きくするCSS
    st.markdown("""
    <style>
    /* カメラ映像を大きく表示 */
        ...
        function switchToRearCamera() {
            ...
            videos.forEach(function(video) {
                ...
                navigator.mediaDevices.getUserMedia({
                    ...
                    console.log('rear camera not available:', e);
                });
            });
        }
        // Streamlit描画後に実行
        setTimeout(switchToRearCamera, 1000);
        // MutationObserverでDOMが追加されたときも実行
        var observer = new MutationObserver(function() { switchToRearCamera(); });
...
with tab3:
    ...
    <div class="info-card">
        ...
        1. <strong>カメラ撮影</strong> タブで信号機を撮影（背面カメラが自動で起動）<br>
        2. または <strong>画像アップロード</strong> タブから写真をアップロード<br>
    ...
    <div class="info-card" style="margin-top:0.8rem">
        ...
        • <strong>物体検出</strong>: YOLO26s (Ultralytics)<br>
    ...
    <div class="info-card" style="margin-top:0.8rem">
        ...
        独自学習した <code>yolo26s_traffic.pt</code> を<br>
...
<div style="text-align:center;color:#6B6B85;font-size:0.8rem;padding-bottom:1rem">
    視覚障害者支援ツール | Built with YOLO26s + OpenCV + Streamlit
</div>
""", unsafe_allow_html=True)
